...
import { initialProducts, categories as MOCK_CATEGORIES } from '@/showcase/data'
...
const initialPrices: PriceItem[] = initialProducts.map((p: any) => ({
  id: String(p.id),
  sku: p.sku,
  name: p.name,
  category: p.category,
      ...
      <div className="mx-auto max-w-screen-2xl space-y-6 px-6 py-6 lg:px-8 xl:px-10">
        ...
        <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
          {[
            ...
            { label: 'Actualizados hoy', value: prices.filter((item) => item.lastUpdated === 'Hoy').length, icon: RefreshCw, variant: 'primary' },
          ].map((stat) => (
